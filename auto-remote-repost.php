...
/*
 ...
 Plugin URI: https://www.webcreative.me/wordpress-plugins/
 Description: Reposts/clones the currently added post on a remote Wordpress website/domain.
 Version: 1.1.0
 Author: Webcreative.me
 Author URI: https://www.webcreative.me
 Text Domain: auto-remote-repost
 */

/*  Copyright 2009-2025	SomeDev (email : somedev@gmx.com)
*/
  ...
  <div class="wrap">
    ...
    <form method="post" action="">
      <table class="form-table">
        <tr valign="top">
          ...
          <b>Notes</b>:<br>
					...
					- In case you chose "Source", add the URL of the <b>remote domain</b>. If you choose "Destination" just paste the generated token below in the remote website.<br>
					- The <b>categories mappings</b> sync the source/local categories and the ones on the destination domain. Click "Show Categories" to list the local/source taxonomies/categories, then the same on the remote domain. This should help you easily get the correspondent IDs of the desired categories you want to match.
          </td>
        </tr>
				<tr>
      ...
      <?php echo $categories; ?>
                ...
                <tr>
  <th scope="row">Show Current/Source Categories:</th>
  <td>
    <button type="button" class="button" id="show_categories_btn">List/Show Post Categories</button>
    <div id="category_list" style="margin-top: 10px;"></div>
  </td>
</tr>
			...
			<script>
                ...
                jQuery("#show_categories_btn").on("click", function() {
                    jQuery.ajax({
                        url: ajaxurl,
                        type: "POST",
                        data: {
                            action: "arrp_fetch_categories"
                        },
                        success: function(response) {
                            jQuery("#category_list").html(response);
                        },
                        error: function() {
                            jQuery("#category_list").html("Failed to fetch categories.");
                        }
                    });
                });
			</script>
...
add_action('wp_ajax_arrp_fetch_categories', 'arrp_fetch_categories');
function arrp_fetch_categories() {
    ...
    if (empty($categories)) {
        ...
        echo '<table class="widefat" style="width:initial;"><thead><tr><th>ID</th><th>Name</th></tr></thead><tbody>';
        foreach ($categories as $cat) {
            echo '<tr><td>' . esc_html($cat->term_id) . '</td><td>' . esc_html($cat->name) . '</td></tr>';
        }
        echo '</tbody></table>';
    }

    wp_die(); // required to properly terminate
}
